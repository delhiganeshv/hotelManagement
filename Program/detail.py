...
class DetailWindow:
    def __init__(self, root):
        ...
        floorEntry = ttk.Entry(
            entryFrame, textvariable=self.floor, width=20, font=("arial", 12, "bold")
        )
        ...
        roomNoEntry = ttk.Entry(
            entryFrame, textvariable=self.roomNo, width=20, font=("arial", 12, "bold")
        )
        ...
        if self.roomNo.get() == "":
            ...
            try:
                ask = messagebox.askyesno(
                    "Confirm", "Do you want to delete this room?", parent=self.root
                )
                if ask > 0:
                    roomDb.remove(self.roomNo.get())
                    messagebox.showinfo(
                        "Success", "Room deleted successfully", parent=self.root
                    )
                    self.displayAll()
            except:
    ...
    def updateRoom(self):
        if (
            ...
            try:
                ask = messagebox.askyesno(
                    "Confirm", "Do you want to update this room?", parent=self.root
                )
                if ask > 0:
                    roomDb.update(
                        self.floor.get(), self.roomNo.get(), self.roomType.get()
                    )
                    messagebox.showinfo(
                        "Success", "Room updated successfully", parent=self.root
                    )
                    self.displayAll()
            except:
                messagebox.showerror("Error", "Room does not exist", parent=self.root)
