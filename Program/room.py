...
class RoomBooking:
    def __init__(self, root):
        ...
        roomTypeEntry = ttk.Combobox(
            labelFrameLeft,
            textvariable=self.roomType,
            width=21,
            state="readonly",
            font=("times new roman", 13, "bold"),
        )
        ...
        availableRoomEntry = Entry(
            labelFrameLeft,
            textvariable=self.availableRoom,
            width=23,
            font=("times new roman", 13, "bold"),
        )
    ...
    def fetchContact(self):
        if self.contact.get() == "":
            ...
            if row == None:
                messagebox.showerror(
                    "Error", "This contact number is not found", parent=self.root
                )
            else:
                # print(row)
    ...
    def addData(self):
        if (
            self.contact.get() == ""
            or self.check_in.get() == ""
            or self.check_out.get() == ""
            or self.roomType.get() == ""
            or self.availableRoom.get() == ""
            or self.meal.get() == ""
            or self.NoOfDays.get() == ""
        ):
            ...
            try:
                ...
                messagebox.showinfo(
                    "Success", "Room Details has been added", parent=self.root
                )
                self.displayAll()
                self.resetData()
    ...
    def updateData(self):
        if (
            ...
            try:
                ...
                messagebox.showinfo(
                    "Success", "Room Details has been updated", parent=self.root
                )
                self.displayAll()
                self.resetData()
            except Exception as es:
    ...
    def deleteData(self):
        if self.contact.get() == "":
            messagebox.showerror(
                "Error", "Customer contact is required", parent=self.root
            )
        else:
            try:
                ask = messagebox.askyesno(
                    "Confirm", "Do you want to delete this customer?", parent=self.root
                )
                if ask > 0:
                    roomDB.remove(self.contact.get())
                    messagebox.showinfo(
                        "Success", "Room Detail has been deleted", parent=self.root
                    )
                    self.displayAll()
            except Exception as es:
                messagebox.showerror(
        ...
        if self.searchVal.get() == "" or self.searchText.get() == "":
            ...
            try:
                ...
                if len(rows) != 0:
                    ...
                    messagebox.showerror(
                        "Error", "No such data found", parent=self.root
                    )
            except Exception as es:
                messagebox.showerror(
                    "Error", f"Some thing went wrong:{str(es)}", parent=self.root
                )
    ...
    def totalBill(self):
        ...
        if self.meal.get() == "Breakfast":
            self.subTotal.set(
                str(
                    "%.2f"
                    % (float(self.subTotal.get()) + float(self.NoOfDays.get()) * 100)
                )
            )
            self.paidTax.set(
                str(
                    "%.2f"
                    % (float(self.paidTax.get()) + (float(self.subTotal.get()) * 0.05))
                )
            )
            ...
            self.subTotal.set(
                str(
                    "%.2f"
                    % (float(self.subTotal.get()) + float(self.NoOfDays.get()) * 200)
                )
            )
            self.paidTax.set(
                str(
        ...
        self.totalCost.set(
            str("%.2f" % (float(self.subTotal.get()) + float(self.paidTax.get())))
        )
